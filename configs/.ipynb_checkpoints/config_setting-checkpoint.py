...
class setting_config:
    ...
    model_config = {
        ...
        'bridge': True,
        'gt_ds': True,
        'use_dpag': True,  # 启用DPAG
        'use_edge_supervision': True,  # 启用边缘监督
        'edge_ch_list': [4, 8, 12, 16, 20, 24],  # 边缘路径通道配置
        'adp_dilations': [1, 2, 4, 6],  # ADP模块的膨胀率配置
        'adp_reduction': 16             # 通道压缩比例
    }
    ...
    # criterion = MultiClassDiceCE(weight_ce=1, weight_dice=1, num_classes=3)
    class_weights = torch.tensor([0.5, 1.0, 2.0], dtype=torch.float32)
